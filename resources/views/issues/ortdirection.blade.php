        ...
        <div class="card">
            ...
            <div class="card-body">
                <table class="table table-striped" id="table1">
                    ...
                    @foreach ($ortissues as $ortissue)
                     @foreach ($ortdirection as $ortdir)

                                    @if ($ortissue->id == $ortdir->ort_activity_id )
                        <tr>
                        <td>{{ $ortdir->id }}</td>
                        @foreach ($directorates as $dir)
                         @if ($dir->id == $ortissue->directorate_id  )
                        <td>    {{ $dir->dir_name}}</td> <!--Get Directorate Name-->
                        @endif
                                @endforeach
                        <td>    {{ $ortissue->activity}}</td>
                        <td>{{ $ortdir->direction }}</td>
                        <td>{{ $ortdir->date }}</td>
